...
public class JPanelCrearCliente extends JPanel {
	...
	private JButton btnRegistrarCliente = new JButton("Registrar Cliente");
	private JLabelGradientColorHorizontal lblCrearCliente;
	...
	public JPanelCrearCliente() {
		...
		lblCrearCliente = new JLabelGradientColorHorizontal();
		lblCrearCliente.setFont(TipoPaneles.jJabelTituloTipo);
		...
		int space = 5;
		Border innerBorder = BorderFactory.createTitledBorder("Crear Cliente");
		Border outerBorder = BorderFactory.createEmptyBorder(space, space,
				space, space);
		setBorder(BorderFactory.createCompoundBorder(innerBorder, outerBorder));

		lblCrearCliente.setText("Crear un nuevo cliente");
		...
		panelCrearCliente = new PanelCrearCliente();
		jScrollPane = new JScrollPane(panelCrearCliente);
		add(jScrollPane, BorderLayout.CENTER);
		 //  create a line border with the specified color and width
        final Border border = BorderFactory.createLineBorder(new Color(115,164,209), 3);
        ...
        text[3].addKeyListener(new KeyAdapter(){
			...
			public void keyTyped(KeyEvent e)
		    {
		       ...
		       if(text[3].getText().length()== limite){
		    	   e.consume();
		    	   JOptionPane.showMessageDialog(null, "no se puede introducir más de 5 digitos");  
		       }
		    }
		 });
        //telefono
		...
		btnRegistrarCliente.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					if (controller.existeCliente(text[1].getText()) == false) {
						// campos no vacios
						if (text[1].getText().length() != 0
								...
								if (rdbtnClienteProveedor.isSelected()) {
									...
									s.setCodigoPostal(Integer.parseInt(text[3].getText()));
						...
						JOptionPane
								.showMessageDialog(null, "Cliente ya existe");
					}
		...
		btnCpOk.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					ev.printStackTrace();
					JOptionPane.showMessageDialog(JPanelCrearCliente.this,
	...
	class PanelCrearCliente extends JPanel {
		private static final long serialVersionUID = 1L;

...
public class JPanelModificarCliente extends JPanel {
	...
	public JPanelModificarCliente() {
		...
		jScrollPane = new JScrollPane(clientePanel);
		controller = new Controller();

		setBackground(Color.white);
		int space = 5;
		Border innerBorder = BorderFactory
				.createTitledBorder("Modificar Datos");
		...
		label[8].setText("DNI");
		label[9].setText("Buscar por nombre de Cliente");
		...
		search.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (!buscarTxt.equals("")) {
					try {
						Cliente c = controller.visualizarCliente2(buscarTxt);
						if (c.getDenominacionSocial() == "") {
							JOptionPane.showMessageDialog(null,
									" El cliente buscado no existe ");
						} else {

							text[1].setText(c.getDenominacionSocial());
							text[2].setText(c.getDireccion());
							text[3].setText(String.valueOf(c.getCodigoPostal()));
							text[4].setText(c.getProvincia());
							text[5].setText(c.getCiudad());
							text[6].setText(String.valueOf(c.getTelefono()));
							text[7].setText(c.getEmail());
				...
				if (!text[3].getText().equals("")) {
					if (validarEmail(email)) {
						if (!ciudad.equals("") || !provincia.equals("")) {
							...
							controller.updateCliente(denomi, dom, codpo,
		...
		boton2.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (!buscarTxt.equals("")) {
					try {
						controller.deleteCliente(buscarTxt);
						JOptionPane
								.showMessageDialog(null, "Cliente eliminado");
						setText();
					} catch (SQLException e1) {
	...
	class BuscarPanel extends JPanel {
		private static final long serialVersionUID = 1L;
		...
		public BuscarPanel() {
			...
			Border titleBorder = BorderFactory
					.createTitledBorder("Buscar Cliente");
	...
	class ClientePanel extends JPanel {
		...
		public ClientePanel() {
			...
			Border spaceBorder = BorderFactory.createEmptyBorder(space, space,
					space, space);
			Border titleBorder = BorderFactory.createTitledBorder("Cliente");
			setBorder(BorderFactory.createCompoundBorder(spaceBorder,
					titleBorder));
